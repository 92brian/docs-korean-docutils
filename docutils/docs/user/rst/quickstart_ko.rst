...
만약 무언가를 특정 공간에 문자 그대로 표현하고 싶다면 "````백 쿼트를 두 번````" 사용하라.
...
표시해줘야한다. "``\*``" (quickref__) 이렇게 별표 바로 전에 백 슬래쉬를 쓰거나
...
만약 제목이 `문자 형식(Text styles)`_, 이와 같이 공백을 포함한다면 "```문자 형식(Text styles)`_``"로 표시해야 한다.
